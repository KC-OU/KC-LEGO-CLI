...
def login():
    """Handle user login."""
    clear_screen()
    cprint("===== KC Retired and Retiring Sets Login =====", Colors.HEADER)
    users = load_users()
    enter_count = 0
    while True:
...
def user_management_menu(admin_user, current_username):
    ...
    while True:
        clear_screen()
        cprint("===== KC Retired and Retiring Sets - User Management =====", Colors.HEADER)
        print("1. List Users")
...
def main_menu(current_user):
    ...
    while True:
        try:
            clear_screen()
            cprint(f"===== KC Retired and Retiring Sets - Logged in as {current_user['first_name']} {current_user['last_name']} ({current_user['role']}) =====", Colors.OKCYAN)
...
# Fix: Robust theme picker (handles nested dicts and subthemes)
                    ...
                    try:
                        ...
                        if 1 <= subchoice <= len(sub):
                            ...
                            if isinstance(ssub, dict):
                                # Go deeper into nested subthemes
                                sub = list(ssub.values())[0]
                                theme = f"{theme} - {list(ssub.keys())[0]}"
                                continue
                            else:
                                return (theme, ssub)
                    except ValueError:
...
def input_with_timeout(prompt, timeout=INACTIVITY_TIMEOUT):
    """
    Input with timeout. Raises TimeoutError if user does not respond in time.
    """
    import sys
    import select
